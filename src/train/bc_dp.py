import os
import random
from src.gym import get_rl_env # NOTE: furniture-bench needs to be before torch&Env imports
from gymnasium import Env # NOTE: furniture-bench needs to be before torch&Env imports
import torch # NOTE: furniture-bench needs to be before torch&Env imports
...
trigger_sync = TriggerWandbSyncHook(communication_dir=os.environ.get("WANDB_OSH_COMM_DIR", _comm_default_dir),)
...
@hydra.main(config_path="../config", config_name="il_dp_state")
def main(cfg: DictConfig):
    ...
    # NOTE: Check if we are continuing a run
    run_exists = False
    ...
    # NOTE: Split the dataset into train and test (effective, meaning that this is after upsampling)
    ...
    cfg.robot_state_dim = dataset.robot_state_dim # NOTE: 🔥 🔥 🔥 from dataset 🔥 🔥 🔥
    cfg.action_dim = dataset.action_dim # NOTE: 🔥 🔥 🔥 from dataset 🔥 🔥 🔥
    if cfg.observation_type == "state":
        cfg.parts_poses_dim = dataset.parts_poses_dim # NOTE: 🔥 🔥 🔥 from dataset 🔥 🔥 🔥
    
    # NOTE: Create the policy network
    actor: Actor = get_actor(cfg, device,)
    actor.set_normalizer(dataset.normalizer)
    actor.to(device)
    
    # NOTE: Set the data path in the cfg object
    cfg.data_path = [str(f) for f in data_path]
    
    # NOTE: Update the cfg object with the action dimension
    cfg.n_episodes = len(dataset.episode_ends)
    cfg.n_samples = dataset.n_samples

    # NOTE: Update the cfg object with the observation dimension
    ...
    # NOTE: Create lists for optimizers and lr schedulers
    ...
    # NOTE: build encoder for image-based
    if cfg.observation_type == "image":
    ...
    # NOTE: In sweeps, the init is ignored, so to make sure that the cfg is saved correctly
    ...
    for epoch_idx in tglobal:
        ...
        if (cfg.training.eval_every > 0 and (epoch_idx + 1) % cfg.training.eval_every == 0):
            ...
            # NOTE: Rollout the policy to verify the success rate
            if (
            ...
            # NOTE: Save the model if the test loss is the best so far
            if (cfg.training.store_best_test_loss_model and test_loss_mean < best_test_loss):
                ...
                # wandb.save(save_path) # NOTE: upload to wandb cloud (offl.)

            # NOTE: Save the model if the success rate is the best so far
            ...
            # NOTE: Run diffusion sampling on a training batch
            if (cfg.training.sample_every > 0 and (epoch_idx + 1) % cfg.training.sample_every == 0):
                with torch.no_grad():
                    ...
                    gt_action = actor.normalizer(train_sampling_batch["action"], "action", forward=False)
                    log_action_mse(epoch_log, "train", pred_action, gt_action)
                    val_sampling_batch = dict_to_device(next(iter(testloader)), device)
                    gt_action = actor.normalizer(val_sampling_batch["action"], "action", forward=False)
                    pred_action = actor.action_pred(val_sampling_batch)
                    log_action_mse(epoch_log, "val", pred_action, gt_action)

            # NOTE:  If using EMA, restore the model
            if cfg.training.ema.use:
                ema.restore()

            # NOTE: Since we now have a new test loss, we can update the early stopper
        ...
        # NOTE: We store the last model at the end of each epoch for better checkpointing
        if cfg.training.store_last_model:
            save_path = str(model_save_dir / f"actor_chkpt_last.pt")
            torch.save(save_dict, save_path)
            # wandb.save(save_path) # NOTE: upload to wandb cloud (offl.)

        # NOTE: If switch is enabled, copy the the shadow to the model at the end of each epoch
        if cfg.training.ema.use and cfg.training.ema.switch:
            ema.copy_to_model()

        # NOTE: Log epoch stats
        ...
        # NOTE: If we are in offline mode, trigger the sync
        if (cfg.wandb.mode == "offline" and (epoch_idx % cfg.wandb.get("osh_sync_interval", 1)) == 0):
            trigger_sync()

        # NOTE: Now that everything is logged and restored, we can check if we need to stop
        if early_stop:
            print(f"Early stopping at epoch {epoch_idx} as test loss did not improve for {early_stopper.patience} epochs.")
            break
