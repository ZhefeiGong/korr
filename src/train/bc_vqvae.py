...
from src.carp.optim.lr_control import filter_params, lr_wd_annealing
from src.carp.svqvae import VQVAE, build_vae_disc
from src.behavior.vqvae import ActionVQVAETrainer
...
@hydra.main(config_path="../config", config_name="il_vqvae_state")
def main(cfg: DictConfig):
    ...
    cfg.wandb.name = cfg.vqvae.act_dim_names[cfg.vqvae.act_dim_sep] + "-action-dim" # build run name
    ...
    ############ Load vqvae and trainer ############
    vae_wo_ddp = build_vae_disc(
        device=device,
        ## encoder | decoder
        V=cfg.vqvae.vocab_size, 
        Cvae=cfg.vqvae.vocab_ch, 
        ch=cfg.vqvae.vch, 
        action_dim=1, # here we train a separate vqvae for each dimension
        num_actions=cfg.vqvae.act_horizon,
        dropout=cfg.vqvae.vdrop,
        ## quant
        beta=cfg.vqvae.vqbeta,
        using_znorm=cfg.vqvae.vqnorm,
        quant_conv_ks=3, # fixed here
        quant_resi=cfg.vqvae.vqresi,
        share_quant_resi=4, # fixed here
        patch_nums=cfg.vqvae.patch_nums,
        vae_init=cfg.vqvae.vae_init,
        vocab_init=cfg.vqvae.vocab_init,
    )
    ...
    beta1, beta2 = map(float, cfg.vqvae.vopt_beta.split('_'))
    opt_clz = {
        'adamw': partial(torch.optim.AdamW, betas=(beta1, beta2), fused=cfg.vqvae.afuse,), # torch >= 2.0
    }[cfg.vqvae.opt.lower().strip()]
    opt_kw = dict(lr=cfg.vqvae.vlr, weight_decay=0)
    print(f'[INIT] optim={opt_clz}, opt_kw={opt_kw}\n')
    ### build the optimizer
    vae_opt = AmpOptimizer(
        mixed_precision=cfg.vqvae.fp16, 
        optimizer=opt_clz(params=para_groups, **opt_kw), 
        names=names,
        paras=paras,
        grad_clip=cfg.vqvae.vclip, # whether to utilize the gradient clipping
        n_gradient_accumulation=cfg.vqvae.ac
    )
    ### build multi-scale action tokenizer
    vqvae_trainer = ActionVQVAETrainer(
        device = device,
        normalizer=dataset.normalizer, # build from dataset
        vae_wo_ddp = vae_wo_ddp,
        vae_opt = vae_opt,
        ema_ratio = cfg.vqvae.vema,
        is_ema = cfg.vqvae.vema>0.0, # True as default
        act_dim_sep = cfg.vqvae.act_dim_sep,
        act_dim_names = cfg.vqvae.act_dim_names,
    )
    ...
    cfg.training.warmup_step = cfg.vqvae.vwp * len(trainloader)
    ...
    for epoch_idx in tglobal:
        ...
        for batch in tepoch:
            ...
            min_vlr, max_vlr, min_vwd, max_vwd = lr_wd_annealing(cfg.vqvae.vsche, 
                                                                 vqvae_trainer.vae_opt.optimizer, 
                                                                 cfg.vqvae.vlr, 
                                                                 cfg.vqvae.vwd, 
                                                                 cfg.vqvae.vwde, 
                                                                 global_step,
                                                                 ...
                                                                 wp0=cfg.vqvae.vwp0, 
                                                                 wpe=cfg.vqvae.vwpe)
            loss, losses_log, usages = vqvae_trainer.compute_loss(batch)
            grad_norm, scale_log2 = vqvae_trainer.backward_update(loss, global_step)
            ...
            # Record the usage of codebook
            for idx, value in enumerate(cfg.vqvae.patch_nums):
                train_losses_log[f"scale_{value}"] = usages[idx]
            train_losses_log[f"scale_all"] = usages[-1]
        ...
        save_dict = {
            "model_state_dict": vqvae_trainer.state_dict(),
        ...
        if (cfg.training.eval_every > 0 and (epoch_idx + 1) % cfg.training.eval_every == 0):
            ...
            for test_batch in test_tepoch:
                with torch.no_grad():
                    ...
                    # get test loss
                    test_loss_val, losses_log, _ = vqvae_trainer.compute_loss(test_batch)
                    # logging test loss
            ...
            if (cfg.training.store_best_test_loss_model and test_loss_mean < best_test_loss):
                ...
                save_path = str(model_save_dir / f"{cfg.vqvae.act_dim_names[cfg.vqvae.act_dim_sep]}_dim_chkpt_best_test_loss.pt")
            ...
            if (cfg.training.checkpoint_interval > 0 and (epoch_idx + 1) % cfg.training.checkpoint_interval == 0):
                save_path = str(model_save_dir / f"{cfg.vqvae.act_dim_names[cfg.vqvae.act_dim_sep]}_dim_chkpt_{epoch_idx}.pt")
                torch.save(save_dict, save_path)
            
        # We store the last model at the end of each epoch for better checkpointing
        if cfg.training.store_last_model:
            save_path = str(model_save_dir / f"{cfg.vqvae.act_dim_names[cfg.vqvae.act_dim_sep]}_dim_chkpt_last.pt")
            torch.save(save_dict, save_path)
