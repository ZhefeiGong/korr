...
from src.behavior.residual_diffusion_dir import ResidualDiffusionPolicy # NOTE: direct rl-dp policy
...
def main(cfg: DictConfig):
    ...
    # NOTE: set up goal reference for different situations
    if "KPM" in cfg.actor.residual_policy._target_:
        ...
        if isinstance(g_goal_path, str) and g_goal_path.endswith(".pkl"): # when goals
            ...
            g_goal_nobs = convert_to_tensor(residual_nobs).to(device) # from array to tensor
    ...
    buf_actions = torch.zeros((steps_per_iteration, cfg.num_envs) + env.action_space.shape) # like: [max_env_steps, num_envs, 10]
    ...
    while global_step < cfg.total_timesteps:
        ...
        for step in range(0, steps_per_iteration):
            ...
            next_residual_nobs = agent.process_obs(next_obs) # proprio. state + parts poses | normalized nobs
            
            buf_dones[step] = next_done # NOTE: storage here
            buf_obs[step] = next_residual_nobs # NOTE: storage here
            ...
            buf_actions[step] = residual_naction.cpu() # NOTE: storage here
            ...
            if step + 1 >= steps_per_iteration:
                ...
                with torch.no_grad():
                    buf_next_obs[-1] = agent.process_obs(next_obs) # state_obs
        ...
        b_actions = buf_actions.reshape((-1,) + env.action_space.shape) # unfold to (num_envs * max_steps, action_space)
        ...
        # Process the obs for the residual policy
        next_value = residual_policy.get_value(agent.process_obs(next_obs)).reshape(1, -1).cpu() # Value Critic
        ...
        for epoch in trange(cfg.update_epochs, desc="Policy update"):
            ...
            for start in range(0, cfg.batch_size, cfg.minibatch_size):
                ...
                with torch.enable_grad():
                    _, newlogprob, entropy, newvalue, action_mean = (residual_policy.get_action_and_value(mb_obs, mb_actions)) # based on old actions
                logratio = newlogprob - mb_logprobs # log(π_θ/π_θ_{old})
                ratio = logratio.exp() # r(θ)
                # 💡 Kullback–Leibler divergence: D_{KL} = (π_θ_{old}∣∣π_θ) = E[r_t − 1 − logr_t]
                with torch.no_grad():
                ...
                loss.backward()
                nn.utils.clip_grad_norm_(residual_policy.parameters(), cfg.max_grad_norm)
        ...
        action_norms = torch.norm(b_actions[:, :3], dim=-1).cpu() # only consider position (xyz)
