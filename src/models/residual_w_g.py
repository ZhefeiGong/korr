...
class ResidualPolicyWGoal(nn.Module, PrintParamCountMixin):
    def __init__(
        self,
        obs_shape,
        action_shape,
        ...
        **kwargs,
        ...
        self.actor_mean = build_mlp(
            input_dim=self.obs_dim + self.sep_obs_dim, # obs + act + goal
    ...
    def set_goal(self, nobs):
        assert nobs.shape[-1] == self.sep_obs_dim, "goal_obs mismatch obs_dim"
        self.goal_obs = nobs # [1, sep_obs_dim,]
    ...
    def get_action_and_value(
        ...
        ngoals = self.goal_obs.expand(nobs.shape[0], -1) # [N, goal_dim]
        action_mean: torch.Tensor = self.actor_mean(torch.cat([nobs, ngoals], dim=-1)) # [N, goal_dim]
...
class BiggerResidualPolicy(ResidualPolicyWGoal):
