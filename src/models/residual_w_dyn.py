...
class ResidualPolicyWDynamics(nn.Module, PrintParamCountMixin):
    def __init__(
        self,
        obs_shape,
        action_shape,
        ...
        **kwargs,
        ...
        self.sep_obs_dim = np.prod(obs_shape) # single observation dimension
        self.sep_act_dim = np.prod(action_shape) # single action dimension
        self.obs_dim = np.prod(obs_shape) + np.prod(action_shape) # condition on `observation` and `action`
        self.action_scale = action_scale

        self.actor_dyn = build_mlp(
            input_dim=self.obs_dim,
            hidden_sizes=[actor_hidden_size] * actor_num_layers,
            output_dim=self.sep_obs_dim,
    ...
    def get_action_and_value(
        ...
        nobs_next_base: torch.Tensor = self.actor_dyn(nobs) # obs + base_action -> nobs_next_base
        action_mean: torch.Tensor = self.actor_mean(nobs_next_base) # nobs_next_base -> res_action
        ...
        return (
            action,
            probs.log_prob(action).sum(dim=1), # logπ_θ(a_t|s_t)
            probs.entropy().sum(dim=1), # H[π_θ(⋅∣s_t)]
            self.critic(nobs), # values from critic
            action_mean,
        )
    ...
    def bc_loss(
        ...
        """
        nobs_next_base: torch.Tensor = self.actor_dyn(res_nobs) # obs + base_action -> nobs_next_base
        action_mean: torch.Tensor = self.actor_mean(nobs_next_base) # nobs_next_base -> res_action
    ...
    def dyn_loss(
        ...
        raw_nobs = nobs[...,:self.sep_obs_dim] # only observation
        raw_next_nobs = next_nobs[...,:self.sep_obs_dim] # only observation
        inp_nobs = torch.cat((raw_nobs, nact), dim=-1) # observation + executable actions
        raw_next_nobs_pred = self.actor_dyn(inp_nobs) # calculate KPM process
        return torch.nn.functional.mse_loss(raw_next_nobs_pred, raw_next_nobs)
...
class BiggerResidualPolicy(ResidualPolicyWDynamics):
