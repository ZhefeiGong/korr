...
class StateDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.action_dim = self.train_data["action"].shape[-1]
        self.robot_state_dim = self.train_data["robot_state"].shape[-1] # NOTE: robot-state dimension
        self.parts_poses_dim = self.train_data["parts_poses"].shape[-1] # NOTE: other-parts dimension
