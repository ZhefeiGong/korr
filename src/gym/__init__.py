...
def get_rl_env(
    ...
    max_env_steps=5_000,
    resize_img=True,
    observation_space="image",  # NOTE: Observation space for the robot. Options are 'image' and 'state'.
    act_rot_repr="quat",
    action_type="pos",  # NOTE: Action type for the robot. Options are 'delta' and 'pos'.
    april_tags=False,
    verbose=False,
    headless=True,
    record=False,
    concat_robot_state=False, # NOTE: Whether concate the robot state
    ctrl_mode="diffik",
    obs_keys=None,
    **kwargs,
):
    ...
    if task == "bimanual_insertion":
        ...
        if obs_keys is None:
            ...
            if observation_space == "state":
                ...
                obs_keys = [key for key in obs_keys if "image" not in key] # NOTE: filter `image` if we don't need

        if action_type == "relative":
            print("[INFO] Using relative actions. This keeps the environment using position actions.")
        ...
        with suppress_all_output(not verbose):
            env = FurnitureRLSimEnv(
                ...
                headless=headless,  # If true, simulation runs without GUI.
                obs_keys=obs_keys, # NOTE: the observation to load
                compute_device_id=gpu_id,
                graphics_device_id=gpu_id,
                init_assembled=False,  # If true, the environment is initialized with assembled furniture.
                np_step_out=False,  # If true, env.step() returns Numpy arrays.
                ...
                max_env_steps=max_env_steps,  # Maximum number of steps per episode.
                act_rot_repr=act_rot_repr,  # Representation of rotation for action space. Options are 'quat' and 'axis'.
                ctrl_mode="diffik",  # Control mode for the robot. Options are 'osc' and 'diffik'. -> Operational Space Control & Differentiable Inverse Kinematics
                action_type=action_type,  # Action type for the robot. Options are 'delta' and 'pos'.
                verbose=verbose,  # If true, prints debug information.
                # **kwargs,
            )
