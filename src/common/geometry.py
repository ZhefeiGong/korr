...
def rot_6d_to_isaac_quat(rot_6d: torch.Tensor) -> torch.Tensor:
    ...
    # ### NOTE: awful error with one more transform from xyzw to wxyz (offl.)
    # # Convert rotation matrix to quaternion
    # quat = matrix_to_quaternion(rot_mats) # Pytorch3D Quaternion
    # # Convert quaternion from PyTorch3D format to IsaacGym format
    # quat = pytorch3d_quat_to_isaac_quat(quat)

    ### NOTE: Convert rotation matrix to quaternion in IsaacGym format
    quat = matrix_to_quaternion_xyzw(rot_mats)
...
def np_action_quat_to_6d_rotation(action: np.ndarray) -> np.ndarray:
    ...
    Parts:
        - 3D position
        - 4D quaternion rotation - isaac_quat
        - 1D gripper
