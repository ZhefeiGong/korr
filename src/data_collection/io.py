...
def save_raw_rollout(
    ...
    if action_type == "pos":
        ...
        delta_action_pos = actions[:, :3] - np.array([rs["ee_pos"] for rs in robot_states[:-1]])

        # Insert the delta quat into the actions
        actions = np.concatenate([delta_action_pos, delta_action_quat.as_quat(), actions[:, -1:]], axis=1)

    data: Trajectory = {
        "observations": observations, # NOTE: List( Dict{ robot_state = List, color_image1 = array(uint8), color_image2 = array(uint8), parts_poses = array(float) } )
        "actions": actions.tolist(), # NOTE: List( action = List(pos+quat+gripper) )
        "rewards": rewards.tolist(), # NOTE: List( reward = float )
        "success": success,
        "task": task,
