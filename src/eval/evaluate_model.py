...
class LocalCheckpointWrapper:
    def __init__(self, checkpoint_path: str):
        ...
        self.name = self.checkpoint_path.stem # NOTE: the name of the ckpt
        self.id = self.name
...
def get_runs(args: argparse.Namespace) -> List[Run]:
    ...
    if args.wt_path:
        run = LocalCheckpointWrapper(args.wt_path) # NOTE: load local ckpt
        runs = [run]
    else:
        api.flush()
        if args.sweep_id:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run-id", type=str, required=False, nargs="*") # NOTE: retrieve ckpt from wandb
    ...
    parser.add_argument("--wandb", action="store_true") # NOTE: whether save to the corresponding wandb | false (offl.)
    ...
    parser.add_argument("--store-video-wandb", action="store_true") # NOTE: whether upload video to wandb (wandb to True first)
    ...
    try:
        while True:
            ...
            runs = get_runs(args) # NOTE: get runs from `wandb` or `local`
            ...
            for run in runs:
                ...
                if args.wt_path:
                    ...
                    actor: Optional[Actor] = load_model_weights(run=run, actor=actor, wt_type=args.wt_type) # NOTE: load from wandb ckpt
                if actor is None:
                    print(
                ...
                # NOTE: load the evaluating environment here
                if env is None:
                    env = get_rl_env(
                        gpu_id=args.gpu,
                        task=args.task,
                ...
                if args.wandb: # NOTE: load to wandb
