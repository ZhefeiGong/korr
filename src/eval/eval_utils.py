...
def replace_path_in_dict(dictionary, old_str, new_str):
    for key, value in dictionary.items():
        if isinstance(value, DictConfig):
            replace_path_in_dict(value, old_str, new_str)
        elif isinstance(value, str) and 'path' in key.lower():
            if old_str in value:
                dictionary[key] = value.replace(old_str, new_str)
        elif isinstance(value, ListConfig) and 'path' in key.lower():
            new_list = []
            for item in value:
                if isinstance(item, str) and old_str in item:
                    new_list.append(item.replace(old_str, new_str))
                else:
