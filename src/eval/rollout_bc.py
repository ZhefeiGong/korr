...
def rollout(
    ...
    robot_states = [TensorDict(video_obs["robot_state"], batch_size=env.num_envs)] # NOTE: robot propri. state
    ...
    parts_poses = [video_obs["parts_poses"].cpu()] # NOTE: other parts' poses
    actions = list()
...
def calculate_success_rate(
    ...
    if save_rollouts:
        ...
        for rollout_idx in trange(
            ...
            ) # NOTE: save the trajectory until the last reward timestep

            n_steps += n_steps_padding
            trim_start_steps = 0 # NOTE: fixed to 0 as tradition
            ...
            if have_img_obs:
                ...
                video = create_in_memory_mp4(video, fps=20) # NOTE: create the rollout mp4 here, only utilized for wandb
            ...
            if rollout_save_dir is not None and (save_failures or success): # NOTE: whether save the failure cases
