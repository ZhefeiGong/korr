...
from src.koopman.ppo_res_wo_g.kpm_lqr import KoopmanLQR
...
class KPMActor(nn.Module):
    def __init__(self,
        ...
        ### inverse dynamics
        self.inverse_dynamics = self._build_mlp(input_dim=self.obs_lift_dim, output_dim=self.act_dim)
    ...
    def forward(self, ninp: torch.Tensor):
        ...
        nact_res_mean = self.inverse_dynamics(next_nobs) # -> only consider the next obs from KPM
