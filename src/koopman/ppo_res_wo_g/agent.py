...
from src.koopman.ppo_res_wo_g.modules import KPMCritic, KPMActor

class KPMResidualPolicyWOGoal(nn.Module, PrintParamCountMixin):
    def __init__(
        self,
        obs_shape,
        action_shape,
        ...
        **kwargs,
        ...
        # actor
        self.actor = KPMActor(
            obs_dim=np.prod(obs_shape), # only condition on `observation` for koopman
    ...
    def get_action_and_value(
            self,
            nobs: torch.Tensor, # include `observation` + `base_action`
