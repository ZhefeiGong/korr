...
@func: 
| the core implementation of Koopman Linear Ruadratic Resolver
@link:
...
class KoopmanLQR(nn.Module):
    def __init__(self, 
        ...
        if self._g_affine is None:
            self._g_affine = nn.Parameter(torch.empty((self._g_dim, self._g_dim)), requires_grad=True) # [obs_dim, obs_dim]
        else:
        ...
        if self._u_affine is None:
            self._u_affine = nn.Parameter(torch.empty((self._g_dim, self._u_dim)), requires_grad=True) # [obs_dim, act_dim]
        else:
