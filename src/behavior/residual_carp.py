...
from src.behavior.carp import Coarse2FineAutoRegressivePolicy
...
from src.models.residual import ResidualPolicy # NOTE: legacy mlp w/o goal
from src.models.residual_w_g import ResidualPolicyWGoal # NOTE: legacy mlp w goal
from src.models.residual_w_dyn import ResidualPolicyWDynamics # legacy mlp w dynamics
from src.koopman.ppo_res.agent import KPMResidualPolicy # NOTE: koopman solver w/ goal
from src.koopman.ppo_res_wo_g.agent import KPMResidualPolicyWOGoal # NOTE: koopman solver w/o goal
...
class ResidualCARP(Coarse2FineAutoRegressivePolicy):
    ...
    def __init__(
        ...
        # NOTE: 
        # What about having a ensemble of residual layers? 
        # They're cheap to compute and we can use them to both improve the
        # performance of the policy and to estimate the uncertainty of the policy.
        
        # self.residual_policy: ResidualPolicy = hydra.utils.instantiate(
        self.residual_policy: Union[ResidualPolicy, 
                                    ResidualPolicyWGoal, 
                                    ResidualPolicyWDynamics, 
                                    KPMResidualPolicy, 
                                    KPMResidualPolicyWOGoal] = hydra.utils.instantiate(
    ...
    def load_base_state_dict(self, path: str):
        ...
        # Load the model `weights` and `normalizer`
        super().load_state_dict(base_state_dict)
        return

    def state_dict(self):
        # State the checkpoint of base policy
        state = super().state_dict()
        # State the checkpoint of residual policy
        state['residual_policy'] = self.residual_policy.state_dict()
        return state

    def load_state_dict(self, state, strict=True, skip_vae=False):
        # Load the checkpoint of base policy
        super().load_state_dict(state)
        # Load the checkpoint of residual policy
        self.residual_policy.load_state_dict(state['residual_policy'], strict, skip_vae)
        return
    ...
    def base_action_normalized(self, obs: Dict[str, torch.Tensor]) -> torch.Tensor:
        action = super().action(obs) # get the base action prediction
        return self.normalizer(action, "action", forward=True) # normalize again
    ...
    def get_action_and_value(
        ...
        eval: bool = False,
    ) -> ResidualTrainingValues: # -> used for residual policy
    ...
    def actor_parameters(self):
        # find parameters without `critic`
        return [p for n, p in self.residual_policy.named_parameters() if "critic" not in n]

    @property
    def critic_parameters(self):
        # find parameters with `critic`
