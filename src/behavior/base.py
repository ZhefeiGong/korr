...
class Actor(torch.nn.Module, PrintParamCountMixin, metaclass=PostInitCaller):
    ...
    def __init__(
        ...
        super().__init__()
        self.normalizer = LinearNormalizer() # NOTE: important for execution
        ...
        if self.observation_type == "image": # NOTE: image-based model
            self._initiate_image_encoder(cfg) # NOTE: load encoder for image-based
            ...
            self.proprioception_dropout = cfg.regularization.get("proprioception_dropout", 0.0)
            self.front_camera_dropout = cfg.regularization.get("front_camera_dropout", 0.0)
            self.vib_front_feature_beta = cfg.regularization.get("vib_front_feature_beta", 0.0)
            self.rescale_loss_for_domain = actor_cfg.get("rescale_loss_for_domain", False)
            self.confusion_loss_anchored = actor_cfg.get("confusion_loss_anchored", False)
            self.weight_confusion_loss_by_action = actor_cfg.get("weight_confusion_loss_by_action", False)

        elif self.observation_type == "state": # NOTE: state-based model
            self.robot_state_dim = cfg.robot_state_dim
            self.parts_poses_dim = cfg.parts_poses_dim
            self.timestep_obs_dim = cfg.robot_state_dim + cfg.parts_poses_dim # obs_dim for state observation
    ...
    def _initiate_image_encoder(self, cfg):
        ...
        self.encoding_dim = self.encoder1.encoding_dim # NOTE: from encoder
        ...
        # NOTE: obs_dim for visual observation
    ...
    def _normalized_obs(self, obs: deque, flatten: bool = True):
        ...
        if self.observation_type == "image": # NOTE: image-based model
            ...
            nobs = torch.cat([nrobot_state, feature1, feature2], dim=-1) # NOTE: proprio. state + vision
            ...
            nobs = torch.cat([nrobot_state, nparts_poses], dim=-1) # NOTE: proprio. state + poses
    ...
    def action(self, obs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Return only the first action in the queue
    ...
    def confusion_loss(self, batch, feature1, feature2):
        ...
        # Indices of sim-data and real-data in each batch
