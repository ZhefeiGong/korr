...
--config-name rl_ppo_carp \
env.task=round_table \
env.randomness=low \
num_env_steps=1000 \
wandb.entity=your_wandb_name \
wandb.name=korr_ppo_state_carp \
debug=false \
checkpoint_interval=100 \
actor.residual_policy._target_=src.koopman.ppo_res_wo_g.agent.KPMResidualPolicyWOGoal \
base_policy.wt_path=/path/to/your/bsae/policy/carp_chkpt_best_success_rate.pt \
actor.residual_policy.g_goal_path=/path/to/your/goal_obs.pkl \


### goal-condition
# actor.residual_policy._target_=src.koopman.ppo_res.agent.KPMResidualPolicy \
